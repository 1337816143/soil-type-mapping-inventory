(function(){
  ...
  if(!Q)return;
  var snapshot=[];
  function clone(item){return{path:item.path,sourcePath:item.sourcePath||item.path,batch:item.batch||'',manualBatch:item.manualBatch,directoryConfirmation:item.directoryConfirmation,manualDataKey:item.manualDataKey,manualAssociation:item.manualAssociation,city:item.city||'',unit:item.unit||'',district:item.district||''}}
    ...
    if(button.dataset.splitV2==='1'&&snapshot.length){
      ...
      Q.state.files=(Q.state.files||[]).map(function(item){
        ...
        return{
          ...
          from:item.from||'',
          directoryConfirmation:source.directoryConfirmation||item.directoryConfirmation,
